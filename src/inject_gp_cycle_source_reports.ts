/**
 * Import GP source reports data into the per-cycle source_reports table.
 * 
 * Input:
 * - CSV: /longterm_sch/reviewed_cycle{N}_source_list_GP_visibility_eachday.csv
 * - Text: /longterm_sch/source_reports/{sourceId}_{sourceName}.txt
 * - Schedule: /longterm_sch/schedule_result.csv
 * 
 * Output:
 * - Populates gp_cycleN_source_reports table with chart data and summaries
 */
...
type DatasetArg = "gp" | "cycle2" | "gf";

function readDatasetArg(): DatasetType {
  const arg = process.argv.find((item) => item.startsWith("--dataset="));
  const fromArg = arg?.split("=")[1]?.trim().toLowerCase() as DatasetArg | undefined;
  const fromEnv = process.env.SOURCE_REPORT_DATASET?.trim().toLowerCase() as DatasetArg | undefined;
  const value = fromArg || fromEnv || "gp";

  if (value === "gf") return "gf";
  // Backward-compatible mapping: legacy dataset partition name is "cycle2".
  if (value === "cycle2") {
    console.warn("[warn] --dataset=cycle2 is legacy naming; prefer --dataset=gp.");
  }
  return "cycle2";
}

function resolveInputs(cycle: number, dataset: DatasetType): {
  // For GP: multiple source CSVs to merge (GP + WXTCAL + FXTCAL share the same table).
  sourceCsvs: string[];
  scheduleCsv: string;
  reportDir: string;
} {
  if (dataset === "gf") {
    return {
      sourceCsvs: [process.env.SOURCE_REPORT_SOURCE_CSV ||
        path.join(LONGTERM_SCH_DIR, `reviewed_cycle${cycle}_source_list_GF_forDatabase.csv`)],
      scheduleCsv: process.env.SOURCE_REPORT_SCHEDULE_CSV ||
        path.join(LONGTERM_SCH_DIR, "schedule_gf_records.csv"),
      reportDir: process.env.SOURCE_REPORT_DIR ||
        path.join(LONGTERM_SCH_DIR, "source_reports_gf"),
    };
  }

  // GP dataset: GP + WXTCAL + FXTCAL are all stored in gp_cycleN, so their
  // source reports must be populated from all three visibility CSVs.
  const gpCsvs = process.env.SOURCE_REPORT_SOURCE_CSV
    ? [process.env.SOURCE_REPORT_SOURCE_CSV]
    : [
        path.join(LONGTERM_SCH_DIR, `reviewed_cycle${cycle}_source_list_GP_visibility_eachday.csv`),
        path.join(LONGTERM_SCH_DIR, `reviewed_cycle${cycle}_source_list_wxtcal_visibility_eachday.csv`),
        path.join(LONGTERM_SCH_DIR, `reviewed_cycle${cycle}_source_list_fxtcal_visibility_eachday.csv`),
      ];

  return {
    sourceCsvs: gpCsvs,
    scheduleCsv: process.env.SOURCE_REPORT_SCHEDULE_CSV ||
      path.join(LONGTERM_SCH_DIR, "schedule_result.csv"),
    reportDir: process.env.SOURCE_REPORT_DIR ||
      path.join(LONGTERM_SCH_DIR, "source_reports"),
  };
}
...
async function main() {
  ...
  const { sourceCsvs, scheduleCsv, reportDir } = resolveInputs(cycle, dataset);

  console.log(`[info] Starting source reports import for dataset=${dataset}...`);

  // Read and merge all source CSVs (GP merges GP + WXTCAL + FXTCAL).
  const sourceRows: SourceRow[] = [];
  for (const sourceCsv of sourceCsvs) {
    if (!fs.existsSync(sourceCsv)) {
      console.warn(`[warn] CSV not found, skipping: ${sourceCsv}`);
      continue;
    }
    const csvContent = fs.readFileSync(sourceCsv, "utf-8");
    const rows = parseCSV(csvContent, { columns: true }) as SourceRow[];
    sourceRows.push(...rows);
    console.log(`[info] Loaded ${rows.length} rows from ${path.basename(sourceCsv)}`);
  }
  if (sourceRows.length === 0) {
    console.error(`[error] No source rows loaded. Check CSV paths.`);
    process.exit(1);
  }
  console.log(`[info] Total source rows: ${sourceRows.length}`);
