...
const WEEKLY_PLANS_DIR = path.join(process.cwd(), "longterm_sch", "weekly_plans_gf");
...
async function main() {
  ...
  const { db, longTermObservationListCycle2GF } = await loadModules(cycle);
  ...
  await db.delete(longTermObservationListCycle2GF);
  ...
  for (let index = 0; index < records.length; index += INSERT_BATCH_SIZE) {
    ...
    const inserted = await db
      .insert(longTermObservationListCycle2GF)
      .values(batch)
      .returning({ id: longTermObservationListCycle2GF.id });
    insertedCount += inserted.length;
  }

  console.log(`Imported ${insertedCount} rows into long_term_observation_list_cycle2_gf from ${fileNames.length} weekly plan files.`);
}
