...
function getSourceCsvPaths(cycle: number): string[] {
  // GP / WXTCAL / FXTCAL are ingested into the shared cycle{n}_skymap_sources table.
  return [
    path.join(LONGTERM_DIR, `reviewed_cycle${cycle}_source_list_GP_visibility_eachday.csv`),
    path.join(LONGTERM_DIR, `reviewed_cycle${cycle}_source_list_wxtcal_visibility_eachday.csv`),
    path.join(LONGTERM_DIR, `reviewed_cycle${cycle}_source_list_fxtcal_visibility_eachday.csv`),
  ];
}
...
async function main() {
  ...
  const sourceCsvPaths = getSourceCsvPaths(cycle);
  const datasetTag = `cycle${cycle}`;
  const skymapSourcesTableName = `cycle${cycle}_skymap_sources`;
  const skymapScheduleTableName = `cycle${cycle}_skymap_schedule`;

  console.log(`[info] Importing ${datasetTag} sky map source and schedule CSV data...`);

  const [sourceCsvRowsNested, scheduleCsvRows] = await Promise.all([
    Promise.all(sourceCsvPaths.map((csvPath) => loadCsvRows<SourceCsvRow>(csvPath))),
    loadCsvRows<ScheduleCsvRow>(SCHEDULE_CSV_PATH),
  ]);
  const sourceCsvRows = sourceCsvRowsNested.flat();

  await db.execute(sql.raw(`
    CREATE TABLE IF NOT EXISTS ${skymapSourcesTableName} (
      ...
      CONSTRAINT ${skymapSourcesTableName}_source_id_unique UNIQUE (source_id)
    )
  `));

  await db.execute(sql.raw(`
    CREATE TABLE IF NOT EXISTS ${skymapScheduleTableName} (
      id serial PRIMARY KEY,
      source_id integer NOT NULL,
      visit_index integer,
      n_visits integer,
  ...
  await db.execute(sql.raw(`CREATE INDEX IF NOT EXISTS ${skymapSourcesTableName}_source_id_idx ON ${skymapSourcesTableName} (source_id)`));
  await db.execute(sql.raw(`CREATE INDEX IF NOT EXISTS ${skymapSourcesTableName}_priority_idx ON ${skymapSourcesTableName} (source_priority)`));
  await db.execute(sql.raw(`CREATE INDEX IF NOT EXISTS ${skymapScheduleTableName}_source_id_idx ON ${skymapScheduleTableName} (source_id)`));
  await db.execute(sql.raw(`CREATE INDEX IF NOT EXISTS ${skymapScheduleTableName}_week_index_idx ON ${skymapScheduleTableName} (week_index)`));
  ...
  await db.delete(cycle2SkymapSchedule).where(sql`${cycle2SkymapSchedule.dataset} = ${datasetTag}`);
  await db.delete(cycle2SkymapSources).where(sql`${cycle2SkymapSources.dataset} = ${datasetTag}`);
