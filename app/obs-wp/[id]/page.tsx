...
export default function ObsWpDetailPage() {
  ...
  const queryMatched = searchParams?.get("matched") ?? "";

  const matchedIds = useMemo(() => {
    const fromQuery = queryMatched
      .split(",")
    ...
    const fallback = Number(id);
    return Number.isInteger(fallback) && fallback > 0 ? [fallback] : [];
  }, [id, queryMatched]);

  const [rows, setRows] = useState<ObsWpRow[]>([]);
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const responses = await Promise.all(
        matchedIds.map(async (obsId) => {
          const response = await fetch(`/api/obs-wp/${obsId}`, { cache: "no-store" });
          const data = (await response.json()) as { row?: ObsWpRow; error?: string };
          if (!response.ok) {
            throw new Error(data.error ?? `Failed to load Obs WP #${obsId}`);
          }
      ...
      setRows(
        responses
          .filter((item): item is ObsWpRow => Boolean(item))
          .map((item) => ({ ...item })),
      );
    } catch (error) {
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_15%_20%,rgba(101,170,221,0.22),transparent_35%),radial-gradient(circle_at_85%_15%,rgba(0,93,151,0.16),transparent_32%),linear-gradient(180deg,#f8fbff_0%,#eef4fb_55%,#e8f0f9_100%)] p-4 text-slate-900 dark:bg-[radial-gradient(circle_at_20%_20%,rgba(101,170,221,0.18),transparent_40%),radial-gradient(circle_at_80%_10%,rgba(0,93,151,0.2),transparent_34%),linear-gradient(180deg,#020617_0%,#061426_100%)] dark:text-slate-100 md:p-8">
      <div className="mx-auto max-w-screen-xl rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold">Obs WP Detail — {titleObsId ?? `Record #${id}`}</h1>
            {rows[0]?.sourceName ? (
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{String(rows[0].sourceName)}</p>
            ) : null}
          </div>
          <button
            type="button"
        ...
        <section className="mt-6 rounded-lg ring-1 ring-slate-200 dark:ring-slate-700">
          <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
            <h2 className="text-base font-semibold">
              Observation Work Package{rows.length > 1 ? `s (${rows.length})` : ""}
            </h2>
          </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.map((currentRow, rowIndex) => {
                const populatedSections = SECTIONS.map((section) => ({
                  ...section,
                  fields: section.fields.filter((field) => field in currentRow),
                ...
                return (
                  <div key={currentRow.id} className="px-5 py-4">
                    ...
                    {populatedSections.map((section) => (
                      <div key={`${currentRow.id}-${section.title}`} className="mb-4 last:mb-0">
