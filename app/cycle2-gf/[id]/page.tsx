...
type FieldInput = Omit<Cycle2GfRow, "id">;
...
function rowToInput(row: Cycle2GfRow): FieldInput {
...
export default function Cycle2GfDetailPage() {
  ...
  const [row, setRow] = useState<Cycle2GfRow | null>(null);
  ...
  useLayoutEffect(() => {
    const cachedSourceName =
      sessionStorage.getItem(`${CYCLE2_GF_DETAIL_TITLE_CACHE_KEY_PREFIX}${id}`)?.trim() ?? "";
  ...
  const loadObsList = useCallback(async () => {
    setObsLoading(true);
    try {
      const res = await fetch(`/api/cycle2-gf/${id}/obs-list${cycleQuery}`, { cache: "no-store" });
  ...
  const loadPlannedList = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/cycle2-gf/${id}/planned-list${cycleQuery}`, { cache: "no-store" });
      const data = (await res.json()) as {
        rows?: PlannedObsRow[];
        error?: string;
      };
  ...
  const loadRow = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/cycle2-gf/${id}${cycleQuery}`, { cache: "no-store" });
      const data = (await res.json()) as { row?: Cycle2GfRow; error?: string };
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(`${CYCLE2_GF_DETAIL_TITLE_CACHE_KEY_PREFIX}${id}`, row.sourceName);
  ...
  async function commitSave() {
    ...
    try {
      ...
      const res = await fetch(`/api/cycle2-gf/${id}${cycleQuery}`, {
        method: "PUT",
    ...
    <>
      ...
      <div className="mx-auto max-w-screen-xl rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold">Cycle2 GF — {titleSourceName}</h1>
            {row?.pi ? (
          ...
          {row?.sourceId ? (
            <div className="border-b border-slate-200 dark:border-slate-700">
              <SourceReportChart
                ...
                allowMissingPlan
                disablePreview
                missingPlanLabel="No planned observations found."
