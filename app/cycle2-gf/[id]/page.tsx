...
export default function Cycle2GfDetailPage() {
  ...
  const loadObsList = useCallback(async () => {
    setObsLoading(true);
    try {
      const res = await fetch(`/api/cycle2-gf/${id}/obs-list`, { cache: "no-store" });
  ...
  const loadPlannedList = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/cycle2-gf/${id}/planned-list`, { cache: "no-store" });
      const data = (await res.json()) as {
        rows?: PlannedObsRow[];
        error?: string;
      };
  ...
  const loadRow = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/cycle2-gf/${id}`, { cache: "no-store" });
  ...
  async function commitSave() {
    ...
    try {
      ...
      const res = await fetch(`/api/cycle2-gf/${id}`, {
        method: "PUT",
