...
export default function GpCalDetailPage() {
  ...
  useLayoutEffect(() => {
    const cachedSourceName =
      sessionStorage.getItem(`${GP_CAL_DETAIL_TITLE_CACHE_KEY_PREFIX}${id}`)?.trim() ?? "";
  ...
  function cacheLongTermDetailSourceName(longTermId: number, sourceName: string | null) {
    if (!sourceName) return;
    sessionStorage.setItem(`${DETAIL_TITLE_CACHE_KEY_PREFIX}${longTermId}`, sourceName);
  }
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(`${GP_CAL_DETAIL_TITLE_CACHE_KEY_PREFIX}${id}`, row.sourceName);
    ...
    <>
      ...
      <div className="mx-auto max-w-screen-xl rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold">{cycleLabel} Calibration — {titleSourceName}</h1>
            {row?.pi ? (
        ...
        {!loading ? !row ? (
          ...
          <section className="mt-6 rounded-lg ring-1 ring-slate-200 dark:ring-slate-700">
            ...
            <form id="gp-cal-detail-form" onSubmit={handleSave}>
