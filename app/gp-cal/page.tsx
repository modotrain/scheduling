...
export default function GpCalPage() {
  ...
  const [piFilter, setPiFilter] = useState<PiFilter>("all");
  ...
  function getSortedAndFiltered(): GpCycle2Row[] {
    ...
    const piFiltered =
      piFilter === "fxt"
        ? rows.filter((r) => r.pi?.toUpperCase() === "FXT")
        : piFilter === "wxt"
          ? rows.filter((r) => r.pi?.toUpperCase() === "WXT")
          : rows;

    const filtered = query
      ? piFiltered.filter((row) =>
  ...
  function cacheDetailSourceName(id: number, sourceName: string | null) {
    ...
    sessionStorage.setItem(`${GP_CAL_DETAIL_TITLE_CACHE_KEY_PREFIX}${id}`, sourceName);
  }
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  ...
                  >
                    {searchText || piFilter !== "all" ? "No matching rows." : "No rows found."}
                  </td>
                </tr>
              ) : (
